...
async function testProductionS3() {
  ...
  try {
    ...
    logInfo('This is normal for Supabase Storage - bucket listing may not be supported');
  ...
  const testBucket = process.env.ORGANIZATION_ICON_BUCKET || 'organization-icon';
  ...
  try {
    ...
    if (error.message.includes('NoSuchBucket')) {
      ...
      logInfo(`Please create the "${testBucket}" bucket in production Supabase Storage`);
      logInfo('Visit: https://supabase.com/dashboard/project/hqjuyemsgndugxyshcrj/storage/buckets');
    }
